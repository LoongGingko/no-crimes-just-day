...
-->
<template>
  <div class="fixed left-0 right-0 top-2 z-50 mx-auto flex max-w-screen-2xl items-start px-4 md:top-5 md:px-6" :class="{ 'pointer-events-none': scrolled }">
    <!-- 左侧按钮（PC） -->
    <template v-if="!isMobile">
      <div class="pointer-events-auto mt-2.5 flex flex-1 items-center justify-center">
        <AnimatePresence>
          <motion.button
            v-if="!isHome"
            :initial="{ opacity: 0 }"
            ...
            class="ncjd-card ncjd-hover rounded-full p-2.5 text-slate-500 backdrop-blur-sm dark:text-slate-400 md:p-[11.5px]"
            @click="goBack()"
            ><ChevronLeft />
          </motion.button>
        </AnimatePresence>
    ...
    <template v-else>
      <AnimatePresence>
        <motion.div
          ...
          :exit="{ maxWidth: '0' }"
          :transition="{ duration: 0.3, ease: 'easeInOut' }"
          class="pointer-events-auto mt-2.5 flex items-center justify-center"
        >
          <motion.button
            v-if="!isHome"
            :initial="{ opacity: 0 }"
    ...
    <nav
      ...
      :class="[isHovering ? 'mt-0 py-5' : 'mt-2.5 py-2.5', { 'flex-1': isMobile }, { 'invisible opacity-0': scrolled }]"
      ref="navRef"
    ...
    <!-- 右侧按钮 -->
    <div class="ml-3 mt-2.5 flex items-center justify-center transition-all duration-500 md:flex-1" :class="{ 'invisible opacity-0': scrolled }">
...
import { scrolled, useScroll } from '@/utils/scroll';
